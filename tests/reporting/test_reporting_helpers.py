...
def create_mock_multivariate_model():
    ...
    psi_samples[:, 0, 1] = np.random.normal(-0.1, 0.05, n_samples)  # A -> B (cannibalization)
...
def example_roi_analysis():
    ...
    print("\nROI by Channel:")
    print(roi_df[["channel", "roi_mean", "roi_hdi_low", "roi_hdi_high", "prob_profitable"]].to_string(index=False))
    
    print("\nInterpretation:")
    for _, row in roi_df.iterrows():
        prob = row["prob_profitable"] * 100
        print(f"  {row['channel']}: ROI = {row['roi_mean']:.2f} "
...
def example_prior_posterior():
    ...
    for c in comparisons:
        ...
        print(f"  {c.parameter}: mean = {c.posterior_mean:.3f}, "
