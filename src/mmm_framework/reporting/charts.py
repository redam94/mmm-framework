...
# =============================================================================
# MODEL FIT CHART WITH GEO SELECTOR
# =============================================================================
...
# =============================================================================
# STACKED AREA DECOMPOSITION CHART WITH GEO SELECTOR
# =============================================================================
...
def create_stacked_area_chart_with_geo_selector(
    ...
    has_geo = (
        geo_names is not None 
        and len(geo_names) > 1 
        and components_by_geo is not None
    )
    ...
    layout = {
        "title": {"text": "Revenue Decomposition: Aggregated (Total)", "font": {"size": 16}},
        "paper_bgcolor": "transparent",
        "plot_bgcolor": "transparent",
        "font": {"family": "Inter, sans-serif", "color": colors.text, "size": 12},
        "margin": {"t": 80, "r": 30, "b": 60, "l": 70},
...
# =============================================================================
# WATERFALL CHART WITH GEO SELECTOR
# =============================================================================
...
def create_waterfall_chart_with_geo_selector(
    ...
    traces.append(create_waterfall_trace(component_totals_agg, "Aggregated", visible=True))
    ...
    layout = {
        "title": {"text": "Contribution Breakdown: Aggregated (Total)", "font": {"size": 16}},
        "paper_bgcolor": "transparent",
        "plot_bgcolor": "transparent",
        "font": {"family": "Inter, sans-serif", "color": colors.text, "size": 12},
...
# =============================================================================
# FIT STATISTICS TABLE WITH GEO SELECTOR (JavaScript-based)
# =============================================================================
...
def create_fit_statistics_with_geo_selector(
    ...
    for key, label in stat_labels.items():
        if key in fit_stats_agg:
            ...
            agg_rows += f'<tr data-geo="agg"><td>{label}</td><td class="mono">{val}</td></tr>'
        ...
        for geo in geo_names:
            ...
            for key, label in stat_labels.items():
                if key in geo_stats:
                    ...
                    geo_rows += f'<tr data-geo="{geo}" style="display: none;"><td>{label}</td><td class="mono">{val}</td></tr>'
