...
def _worker_process(
    ...
    try:
        ...
        _update_progress(job_dir, "building", 10, 100, "Building trend configuration...")
        ...
        try:
            _update_progress(job_dir, "contributions", 97, 100, "Computing contributions...")
...
class JobManager:
    ...
    def _list_job_ids(self) -> list[str]:
        ...
        return [
            d.name for d in self.jobs_dir.iterdir()
...
def get_job_manager(jobs_dir: Path | str | None = None) -> JobManager:
    ...
    if _default_manager is None or (jobs_dir and Path(jobs_dir) != _default_manager.jobs_dir):
