            ...
            async for event in stream:
                ...
                for node_name, state_update in event.items():
                    ...
                    for art_key, art_kind in (
                        ...
                        ("prefit_report_path", "prefit_report"),
                        ("results_report_path", "results_report"),
                        ("model_defense_path", "model_defense"),
                        ("slide_deck_path", "slide_deck"),
                    ):
...
async def list_model_validations(project_id: str, thread_id: str | None = None):
    ...
    if sessions_store.get_project(project_id) is None:
        raise HTTPException(status_code=404, detail=f"Project not found: {project_id}")
    arts = sessions_store.list_artifacts(f"__valjobs__{project_id}")
    rows = []
    for a in arts:
    ...
    return JSONResponse(content={"validations": rows, "total": len(rows)})
...
def _serve_report(
    ...
    *,
    ...
    if not os.path.exists(p):
        # Read-side fallback for reports generated before storage became
        # session-scoped (dev wrote fixed names to the server CWD).
        legacy = _ws.legacy_report_path(name)
...
@app.get("/prefit-report", dependencies=[_rep_read])
async def view_prefit_report(thread_id: str | None = None):
    """Serve the pre-fit Model Design Readout (priors, assumptions, prior
    ...
    return _serve_report(
        ...
        thread_id=thread_id,
        missing=(
            "No model design readout yet. Ask the agent to "
            "generate_model_design_readout before fitting."
        ),
    )


@app.get("/prefit-report/download", dependencies=[_rep_read])
async def download_prefit_report(thread_id: str | None = None):
    return _serve_report(
        "agent_prefit_readout.html",
        thread_id=thread_id,
        missing="No model design readout yet.",
        media_type="application/octet-stream",
        download_name="model_design_readout.html",
    )


@app.get("/results-report", dependencies=[_rep_read])
async def view_results_report(thread_id: str | None = None):
    """Serve the interactive MMM Results Report (embedded-posterior,
    recompute-in-browser) generated by the agent's
    ``generate_interactive_report`` tool."""
    return _serve_report(
        "agent_results_report.html",
        thread_id=thread_id,
        missing=(
            "No interactive results report yet. Ask the agent to "
            "generate_interactive_report after fitting a model."
        ),
    )


@app.get("/results-report/download", dependencies=[_rep_read])
async def download_results_report(thread_id: str | None = None):
    return _serve_report(
        "agent_results_report.html",
        thread_id=thread_id,
        missing="No interactive results report yet.",
        media_type="application/octet-stream",
        download_name="mmm_results_report.html",
    )
...
@app.get("/model-defense/download", dependencies=[_rep_read])
async def download_model_defense(thread_id: str | None = None):
    return _serve_report(
        "agent_model_defense.html",
        thread_id=thread_id,
        missing="No model-defense report yet.",
        media_type="application/octet-stream",
        download_name="mmm_model_defense.html",
    )
