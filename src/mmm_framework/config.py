...
class PriorConfig(BaseModel):
    ...
    @classmethod
    def half_normal(cls, sigma: float = 1.0, dims: str | list[str] | None = None) -> PriorConfig:
        return cls(distribution=PriorType.HALF_NORMAL, params={"sigma": sigma}, dims=dims)
    
    @classmethod
    def gamma(cls, alpha: float = 2.0, beta: float = 1.0, dims: str | list[str] | None = None) -> PriorConfig:
        return cls(distribution=PriorType.GAMMA, params={"alpha": alpha, "beta": beta}, dims=dims)
    
    @classmethod
    def beta(cls, alpha: float = 2.0, beta: float = 2.0, dims: str | list[str] | None = None) -> PriorConfig:
        return cls(distribution=PriorType.BETA, params={"alpha": alpha, "beta": beta}, dims=dims)
...
class AdstockConfig(BaseModel):
    ...
    @classmethod
    def geometric(cls, l_max: int = 8, alpha_prior: PriorConfig | None = None) -> AdstockConfig:
        return cls(
...
class VariableConfig(BaseModel):
    ...
    name: str = Field(..., description="Variable name as it appears in VariableName column")
    role: VariableRole
...
class MFFColumnConfig(BaseModel):
    ...
    def dimension_columns(self) -> list[str]:
        ...
        return [self.period, self.geography, self.product, 
                self.campaign, self.outlet, self.creative]
...
class MFFConfig(BaseModel):
    ...
    alignment: DimensionAlignmentConfig = Field(default_factory=DimensionAlignmentConfig)
...
class ModelConfig(BaseModel):
    ...
    # Control selection
    control_selection: ControlSelectionConfig = Field(default_factory=ControlSelectionConfig)
...
def create_social_platform_configs(
    ...
    for platform in platforms:
        configs.append(MediaChannelConfig(
            name=platform,
...
def create_simple_mff_config(
    ...
    return MFFConfig(
        ...
        media_channels=[
            create_national_media_config(name) for name in media_names
        ],
        controls=[
