...
class MMMResults:
    ...
    y_mean: float = 0.0
    y_std: float = 1.0
    # Declarative estimands realized from the posterior at fit time, keyed by
    # estimand name (wildcard-channel estimands expand to "{name}:{channel}").
    # Populated best-effort when the model declares estimands; otherwise empty
    # (use ``model.evaluate_estimands()`` on demand). See mmm_framework.estimands.
    estimands: "dict[str, EstimandResult]" = field(default_factory=dict)
