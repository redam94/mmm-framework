...
It is idempotent and safe to re-run after adding or editing pages: a run on an
unmodified tree rewrites nothing. That is enforced by shared/seo-manifest.json,
which records each page's content hash and the date last stamped on it. A page
is re-dated only when its own content changed — see dates_and_idempotency()
below for why deriving dateModified from git commit dates alone could not work.
Three jobs:
...
"""
import argparse, datetime, glob, hashlib, html, json, os, re, subprocess
...
MANIFEST_PATH = "shared/seo-manifest.json"
#: Date stamped on pages whose content changed this run. Overridable with
#: --stamp / $SEO_STAMP so a rebuild can be pinned (or replayed) to a date.
TODAY = os.environ.get("SEO_STAMP") or datetime.date.today().isoformat()
...
def write_if_changed(path, text):
    """Write only on a real change, so a no-op run leaves an empty diff."""
    try:
...
def dates_and_idempotency():
    """Why the manifest exists (docs for humans, not called by anything).

    ``dateModified`` used to come from ``git log -1 --format=%cs -- <path>`` on
    every run, which self-perpetuates: the run rewrites the page, that rewrite
    gets committed, the commit moves the file's git date, and the next run
    rewrites the page again to the new date. One edited page therefore produced
    date-only diffs across the whole site (78 pages on the v1.1.0 pass), and
    every contributor had to hand-revert them. Running before committing made
    it worse — the page got stamped with its *previous* commit's date.

    So the date is now a recorded fact rather than a derived one: the manifest
    stores a hash of each page's content with the injected block removed, and
    the date last stamped. Unchanged content keeps its date; changed content is
    stamped TODAY (the edit is happening now, which is what git could not
    know pre-commit). Nothing is written when the regenerated page is
    byte-identical, so a second consecutive run is always a no-op.
    """


def content_hash(base):
    """Hash of a page with the injected SEO block removed."""
...
def save_manifest(pages):
    ...
    payload = {
        "_comment": "Generated by tools/build_seo.py — content hash + stamped "
                    "dateModified per page. Keeps the build idempotent; do not "
                    "hand-edit.",
...
def stamped_date(s):
    """The dateModified already injected into a page, if any."""
...
def build_block(f, s, modified):
    ...
    node, is_faq = page_node(f, html.unescape(title), html.unescape(desc), canon, img, modified,
...
def augment_pages(manifest):
    added = rewritten = untouched = 0
    for f in sorted(glob.glob("*.html")):
        ...
        if "</head>" not in s:
            continue
        had = SENTINEL in s
        # Rebuild from current <head>/<body> so real edits propagate.
        base = strip_block(s) if had else s
        digest = content_hash(base)
        prev = manifest.get(f) or {}

        if prev.get("hash") == digest:
            modified = prev.get("modified") or stamped_date(s) or git_date(f)
        elif not prev and had:
            # First run under the manifest: adopt whatever date the page already
            # carries, so introducing the manifest does not re-date the site.
            modified = stamped_date(s) or git_date(f)
        else:
            modified = TODAY  # content genuinely changed (or a brand-new page)

        manifest[f] = {"hash": digest, "modified": modified}
        out = base.replace("</head>", build_block(f, base, modified) + "</head>", 1)
        if out == s:
            untouched += 1
            continue
        ...
        rewritten += had
        added += not had
    print(f"augment_pages: {added} newly augmented, {rewritten} rewritten, "
          f"{untouched} unchanged")


def build_sitemap(manifest):
    def page_date(f):
        """Stamped date when we have one; git date for un-augmented artifacts."""
        return (manifest.get(f) or {}).get("modified") or git_date(f)
    ...
    out = ['<?xml version="1.0" encoding="UTF-8"?>',
           ...
           url(f"{SITE}/index.html", page_date("index.html"), "1.0")]
    for f in pages:
        if f == "index.html": continue
        out.append(url(f"{SITE}/{f}", page_date(f), "0.8" if f in key else None))
    for a in artifacts:
        out.append(url(f"{SITE}/{a}", page_date(a)))
    out.append("</urlset>")
...
GROUPS = [
    ...
    ("Platform (Augur)", ["platform-overview.html", "model-garden.html", "finance-planning.html", "pricing.html", "data-connections.html", "trust.html", "security.html", "responsible-disclosure.html", "evaluator.html"]),
...
SERIES = [
    ...
    ("Modern measurement research (blog)", [
        ...
        "blog-causal-estimates-observational.html",
        "blog-table-2-fallacy.html",
        ...
        "blog-platform-divergent-delivery.html",
        "blog-synthetic-control.html",
        "blog-staggered-did.html",
        "blog-pretrends-testing.html",
        "blog-causalimpact-bsts.html",
        "blog-bayesian-mmm-carryover-shape.html",
        "blog-saturation-or-fatigue.html",
        "blog-aggregation-bends-the-curve.html",
        "blog-holdout-error-mmm.html",
        "blog-calibrate-validate-same-experiment.html",
        "blog-baseline-regularization-confounding.html",
        "blog-advertising-is-a-stock.html",
        "blog-optimizers-curse.html",
        "blog-curve-assumed-you-wouldnt-act.html",
        "blog-table-2-fallacy-mmm.html",
        "blog-modelled-one-p.html",
        "blog-surrogate-outcomes.html",
        "blog-calibrating-mmm-experiments.html",
        "blog-carryover-experiment-timing.html",
        "blog-modeling-pitfalls.html",
        "blog-multiple-comparisons.html",
        ...
        "blog-lindley-to-dad.html",
        "blog-geo-holdout-eig.html",
        "blog-bed-bo-bandits.html",
        "blog-thompson-sampling.html",
        "blog-continuous-learning-interactions.html",
        "blog-vendor-graded-homework.html",
        "blog-prior-was-doing-more.html",
    ]),
...
def main():
    ...
    ap.add_argument("--stamp", metavar="YYYY-MM-DD", default=TODAY,
                    help="dateModified for pages whose content changed "
                         f"(default: today, {TODAY})")
    args = ap.parse_args()
    if not re.fullmatch(r"\d{4}-\d{2}-\d{2}", args.stamp):
        ap.error(f"--stamp must be YYYY-MM-DD, got {args.stamp!r}")
    TODAY = args.stamp

    manifest = load_manifest()
    augment_pages(manifest)
    # Drop entries for pages that no longer exist.
    for gone in [f for f in manifest if not os.path.exists(f)]:
        del manifest[gone]
