...
export function PythonOutputBlock({ output, hasError }: { output: string; hasError: boolean }) {
  ...
  if (kind === 'table') {
    ...
    if (parsed) {
      return (
        <div className="rounded-b-lg border border-t-0 border-line-200 bg-white overflow-hidden">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-cream-50 border-b border-line-200">
            <span className="w-1.5 h-1.5 rounded-full bg-blue-400" />
            <span className="text-[10px] font-semibold text-ink-400 uppercase tracking-widest">DataFrame</span>
            <span className="ml-auto text-[10px] text-ink-300">{parsed.rows.length} rows × {parsed.headers.length} cols</span>
          </div>
          <div className="overflow-x-auto max-h-56">
            <table className="w-full text-[11px] font-mono">
              <thead className="sticky top-0 bg-cream-50 border-b border-line-200">
                <tr>
                  {parsed.headers.map((h, i) => (
                    <th key={i} className="px-3 py-1.5 text-right font-semibold text-ink-600 whitespace-nowrap first:text-left">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-line-200">
                {parsed.rows.map((row, i) => (
                  <tr key={i} className="hover:bg-blue-50/40">
                    {row.map((cell, j) => (
                      <td key={j} className="px-3 py-1 text-right text-ink-700 whitespace-nowrap first:text-left first:font-medium first:text-ink-400">{cell}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
  ...
  return (
    <div className="rounded-b-lg border border-t-0 border-gray-700 bg-gray-950 overflow-hidden">
      <div className="flex items-center gap-2 px-3 py-1.5 bg-ink-700 border-b border-gray-700">
        <span className="w-1.5 h-1.5 rounded-full bg-green-400" />
        <span className="text-[10px] font-semibold text-ink-300 uppercase tracking-widest">Output</span>
      </div>
