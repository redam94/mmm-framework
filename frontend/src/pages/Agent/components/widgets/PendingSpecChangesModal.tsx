    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
        ...
        <div className="px-5 py-4 space-y-3 max-h-[60vh] overflow-y-auto">
          ...
          {changes.map((c) => (
            <div key={c.path} className="rounded-xl border border-gray-200 p-3">
              <div className="flex items-center gap-1.5 mb-1.5">
                <Lock size={11} className="text-amber-500 shrink-0" />
                <span className="text-xs font-semibold text-gray-800">{lockPathLabel(c.path)}</span>
              </div>
              ...
              {c.reason && <p className="text-[11px] text-gray-500 italic mb-2">“{c.reason}”</p>}
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => onResolve(c.path, 'reject')}
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-600 text-xs font-medium rounded-lg border border-gray-200 transition-colors">
                  <X size={12} /> Keep mine
                </button>
              </div>
            </div>
