...
| `execute_python` kernel mode | `execute_python` runs behind a `KernelManager` (`agents/kernels.py`). Default `MMM_AGENT_KERNEL=inprocess` (the in-process warm namespace; `mmm`/`results` available after a fit). Set `MMM_AGENT_KERNEL=subprocess` for one isolated `ipykernel` per session — needs no extra service (`jupyter_client`/`ipykernel` are deps), but in this phase `mmm`/`results` are NOT bound in the subprocess (fits run in the API process), so post-fit `execute_python` referencing `mmm` raises `NameError`. Tunables (subprocess): `MMM_MAX_KERNELS` (live-kernel LRU cap, default 8), `MMM_CELL_TIMEOUT` (per-cell wall-clock seconds before interrupt→kill, default 600), `MMM_KERNEL_RECV_TIMEOUT`/`MMM_KERNEL_READY_TIMEOUT`. See `technical-docs/agent-session-kernels.md` + `-phase1.md`. |
