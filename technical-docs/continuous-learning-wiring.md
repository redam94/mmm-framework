...
* **Host-side compute, shared service.** CL is model-free (no fitted-MMM kernel
  ...
  bespoke job worker (precedent: `_run_validation_job`, `server/src/mmm_framework_server/main.py` —
...
### 3.5 REST endpoints (`server/src/mmm_framework_server/main.py`; all `dependencies=[_proj_read]` reads / `[_proj_write]` writes; 404-check project; `safe_json_dumps_load` responses)
