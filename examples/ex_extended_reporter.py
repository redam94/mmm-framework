...
def create_extended_mmm_data() -> MMMDataBundle:
    ...
    for comp, total in component_totals.items():
        ...
        if comp == "Baseline":
            ...
            component_ts[comp] = weekly_avg * (1 + 0.8 * np.sin(2 * np.pi * np.arange(n_weeks) / 52))
        else:
            # Media channels: some seasonality + random variation
            phase = np.random.random() * 2 * np.pi
            component_ts[comp] = weekly_avg * (1 + 0.25 * np.sin(2 * np.pi * np.arange(n_weeks) / 52 + phase))
    ...
    geo_roi = {}
    roi_multipliers = {
        "Northeast": {"TV": 0.9, "Paid_Search": 1.1, "Paid_Social": 0.95, "Display": 1.0, "Radio": 0.8},
        "Southeast": {"TV": 1.15, "Paid_Search": 0.95, "Paid_Social": 1.1, "Display": 0.9, "Radio": 1.2},
        "Midwest": {"TV": 1.2, "Paid_Search": 0.85, "Paid_Social": 0.8, "Display": 0.85, "Radio": 1.35},
        "Southwest": {"TV": 1.0, "Paid_Search": 1.25, "Paid_Social": 1.2, "Display": 1.15, "Radio": 0.9},
        "West": {"TV": 0.85, "Paid_Search": 1.3, "Paid_Social": 1.25, "Display": 1.2, "Radio": 0.7},
    }
    ...
    mediator_time_series = {
        "Awareness": 45 + 15 * np.sin(2 * np.pi * np.arange(n_weeks) / 52) + np.random.normal(0, 3, n_weeks),
        "Consideration": 28 + 8 * np.sin(2 * np.pi * np.arange(n_weeks) / 52 + 0.5) + np.random.normal(0, 2, n_weeks),
    }
    
    # Ensure values stay in reasonable range
    mediator_time_series["Awareness"] = np.clip(mediator_time_series["Awareness"], 20, 80)
    mediator_time_series["Consideration"] = np.clip(mediator_time_series["Consideration"], 10, 50)
    ...
    cannibalization_matrix = {
        "Core": {
            "Core": {"mean": 1.00, "lower": 0.85, "upper": 1.15},     # Own effect (normalized)
            "Premium": {"mean": -0.12, "lower": -0.22, "upper": -0.02},  # Cannibalizes Premium
            "Value": {"mean": -0.08, "lower": -0.18, "upper": 0.02},     # Slight cannib of Value
        },
        "Premium": {
            "Core": {"mean": -0.18, "lower": -0.28, "upper": -0.08},     # Cannibalizes Core strongly
            "Premium": {"mean": 1.00, "lower": 0.82, "upper": 1.18},     # Own effect
            "Value": {"mean": 0.05, "lower": -0.05, "upper": 0.15},      # Slight halo to Value
        },
        "Value": {
            "Core": {"mean": 0.08, "lower": -0.02, "upper": 0.18},       # Slight halo to Core
            "Premium": {"mean": 0.02, "lower": -0.08, "upper": 0.12},    # Negligible effect
    ...
    marketing_total = sum(v for k, v in component_totals.items() 
...
def generate_extended_report():
    ...
    print(f"  • Marketing Contribution: {data.marketing_contribution_pct['mean']*100:.1f}% "
          f"({data.marketing_contribution_pct['lower']*100:.1f}% - "
          f"{data.marketing_contribution_pct['upper']*100:.1f}%)")
    print(f"  • Blended ROI: {data.blended_roi['mean']:.2f}x "
    ...
    for geo in data.geo_names:
        ...
        rev = perf['revenue']['mean'] if isinstance(perf['revenue'], dict) else perf['revenue']
        roi = perf['blended_roi']['mean'] if isinstance(perf.get('blended_roi', {}), dict) else perf.get('blended_roi', 0)
        print(f"  • {geo}: ${rev/1e9:.2f}B revenue, "
    ...
    for ch in data.channel_names:
        ...
        print(f"  • {ch}: Direct {pathways['_direct']*100:.0f}%, "
    ...
    for prod in data.product_names:
        ...
        print(f"  • {prod}: Direct ${effects['direct']/1e6:.0f}M, "
