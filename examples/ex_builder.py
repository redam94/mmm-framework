...
def example_prior_builders():
    ...
    positive_prior = (
        PriorConfigBuilder().half_normal(sigma=2.0).with_dims("channel").build()
    )
    print(
        f"Positive prior: {positive_prior.distribution.value}(sigma={positive_prior.params['sigma']})"
    )

    # Beta for decay rates (bounded 0-1)
    decay_prior = PriorConfigBuilder().beta(alpha=1, beta=3).build()
    print(
        f"Decay prior: {decay_prior.distribution.value}(α={decay_prior.params['alpha']}, β={decay_prior.params['beta']})"
    )
    ...
    informed_prior = (
        PriorConfigBuilder().truncated_normal(mu=0.05, sigma=0.02, lower=0).build()
    )
    print(
        f"Informed prior: {informed_prior.distribution.value}(μ={informed_prior.params['mu']})"
    )
...
def example_transformation_builders():
    ...
    print(
        f"Saturation: {saturation.type.value}, kappa_bounds={saturation.kappa_bounds_percentiles}"
    )
    ...
    print(
        f"No transforms: adstock={no_adstock.type.value}, saturation={no_saturation.type.value}"
    )
...
def example_media_builders():
    ...
    digital = (
        ...
        .national()
        .with_adstock_builder(AdstockConfigBuilder().geometric().with_max_lag(4))
    ...
    meta = (
        MediaChannelConfigBuilder("Meta")
        .national()
    ...
    snapchat = (
        MediaChannelConfigBuilder("Snapchat")
        .national()
        .with_parent_channel("Social")
        .with_split_dimensions(DimensionType.OUTLET)
        .with_geometric_adstock(4)
    ...
    print(
        f"Meta: parent={meta.parent_channel}, splits={[d.value for d in meta.split_dimensions]}"
    )
...
def example_control_builders():
    ...
    # Geo-level control
    competitor = (
        ControlVariableConfigBuilder("CompetitorPrice")
        .by_geo()
        .allow_negative()
        .build()
    )
...
def example_kpi_builders():
    ...
    print(
        f"National Sales: dims={national_sales.dim_names}, log={national_sales.log_transform}"
    )

    # Geo-level multiplicative
    geo_revenue = (
        KPIConfigBuilder("Revenue")
        .by_geo()
        .multiplicative()
    ...
    # Full granularity
    full_kpi = KPIConfigBuilder("Sales").by_geo_and_product().additive().build()
...
def example_model_builders():
    ...
    print(
        f"Hierarchical: enabled={hierarchical.enabled}, geo_pooling={hierarchical.pool_across_geo}"
    )

    # Seasonality
    seasonality = (
        SeasonalityConfigBuilder().with_yearly(order=2).with_monthly(order=1).build()
    )
    print(f"Seasonality: yearly={seasonality.yearly}, monthly={seasonality.monthly}")

    # Control selection with horseshoe
    control_selection = (
        ControlSelectionConfigBuilder().horseshoe(expected_nonzero=3).build()
    )
    ...
    print(
        f"Bayesian model: method={bayesian_model.inference_method.value}, "
...
def example_complete_mff_builder():
    ...
    config = (
        MFFConfigBuilder()
        # KPI
        .with_kpi_builder(KPIConfigBuilder("Sales").by_geo().additive())
        # Media channels
        .add_media_builder(
            MediaChannelConfigBuilder("TV")
            .national()
        ...
        .add_control_builder(
            ControlVariableConfigBuilder("Distribution").national().positive_only()
        )
        # Alignment
        .with_alignment_builder(
            DimensionAlignmentConfigBuilder().geo_by_sales().product_by_sales()
        )
        # Data settings
...
def example_builder_styles():
    ...
    config_explicit = (
        ...
        .add_control_builder(
            ...
            .with_coefficient_prior(
                PriorConfigBuilder().normal(mu=-0.5, sigma=0.5).build()
            )
        )
        .with_alignment_builder(DimensionAlignmentConfigBuilder().geo_by_population())
        .with_date_format("%Y-%m-%d")
        .weekly()
        .build()
    )
    ...
    print(
        f"  TV adstock prior: {config_explicit.media_channels[0].adstock.alpha_prior.distribution.value}"
    )
...
def example_builder_with_data():
    ...
    config = (
        ...
        .add_control_builder(
            ControlVariableConfigBuilder("Price").national().allow_negative()
        )
        .add_control_builder(
            ControlVariableConfigBuilder("Distribution").national().positive_only()
        )
        .with_alignment_builder(DimensionAlignmentConfigBuilder().geo_by_sales())
        .build()
    )
    ...
    model_config = (
        ...
        .with_hierarchical_builder(
            HierarchicalConfigBuilder().enabled().pool_across_geo().use_non_centered()
        )
