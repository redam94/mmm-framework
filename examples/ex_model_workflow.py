...
def generate_synthetic_mff(
    ...
    if is_national:
        ...
        for i, date in enumerate(dates):
            ...
            tv_spend = (
                80000 * (1.5 if week_idx % 8 < 4 else 0.5) * np.random.lognormal(0, 0.2)
            )
            digital_spend = (
                50000 * (1 + 0.002 * week_idx) * np.random.lognormal(0, 0.15)
            )
            social_spend = 30000 * seasonality * np.random.lognormal(0, 0.2)
            radio_spend = (
                15000 * np.random.lognormal(0, 0.3) if np.random.random() > 0.2 else 0
            )

            # Controls
            price_index = (
                100 + 5 * np.sin(2 * np.pi * week_idx / 26) + np.random.normal(0, 2)
            )
            distribution = (
                85 + 10 * (1 - np.exp(-week_idx / 50)) + np.random.normal(0, 2)
            )
            ...
            records.append(
                {**base, "VariableName": "Digital", "VariableValue": digital_spend}
            )
            records.append(
                {**base, "VariableName": "Paid_Social", "VariableValue": social_spend}
            )
            records.append(
                {**base, "VariableName": "Radio", "VariableValue": radio_spend}
            )

            # Controls
            records.append(
                {**base, "VariableName": "Price_Index", "VariableValue": price_index}
            )
            records.append(
                {**base, "VariableName": "Distribution", "VariableValue": distribution}
            )
        ...
        for i, date in enumerate(dates):
            ...
            records.append(
                {**base_national, "VariableName": "TV", "VariableValue": tv_spend}
            )
            records.append(
                {
                    **base_national,
                    "VariableName": "Digital",
                    "VariableValue": digital_spend,
                }
            )
            records.append(
                {
                    **base_national,
                    "VariableName": "Paid_Social",
                    "VariableValue": social_spend,
                }
            )
            ...
            for geo in geos:
                ...
                media_lift = (
                    tv_contribution + digital_contribution + social_contribution
                ) * 1000
                ...
                # KPI at geo level
                records.append(
                    {**base_geo, "VariableName": "Sales", "VariableValue": sales}
                )
                ...
                records.append(
                    {
                        **base_geo,
                        "VariableName": "Price_Index",
                        "VariableValue": price_index,
                    }
                )
                records.append(
                    {
...
def create_mff_config_national() -> "MFFConfig":
    ...
    config = (
        ...
        .add_media_builder(
            ...
            .with_coefficient_prior(PriorConfigBuilder().half_normal(sigma=2.0).build())
        )
        .add_media_builder(
        ...
        .add_control_builder(
            ...
            .national()
            .with_coefficient_prior(PriorConfigBuilder().half_normal(sigma=1.0).build())
...
def create_mff_config_geo(geo_names: list[str]) -> "MFFConfig":
    ...
    config = (
        ...
        # KPI at geo level
        .with_kpi_builder(
            KPIConfigBuilder("Sales").by_geo().additive()  # Geo-level outcome
        )
        ...
        .add_control_builder(ControlVariableConfigBuilder("Distribution").by_geo())
        # Allocation method for national media to geos
        .with_alignment_builder(
            DimensionAlignmentConfigBuilder().geo_by_population()  # Allocate by population weights
        )
        .weekly()
        .build()
    )
...
def create_model_config(
    ...
    builder.with_seasonality_builder(
        SeasonalityConfigBuilder().with_yearly(
            order=4
...
def compute_all_metrics(
    ...
    converged = (
        metrics["diagnostics"]["divergences"] == 0
        and metrics["diagnostics"]["rhat_max"] < 1.01
        and metrics["diagnostics"]["ess_bulk_min"] > 400
    )
    ...
    if not converged:
        ...
        print(
            "     Consider: more tuning steps, higher target_accept, or simpler model"
        )
    ...
    try:
        roi_df = compute_roi_with_uncertainty(
            mmm, spend_data=spend_data, hdi_prob=hdi_prob
        )
        metrics["roi"] = roi_df
        ...
        for _, row in roi_df.iterrows():
            ...
            print(
                f"  {row['channel']:<15} {row['roi_mean']:>8.2f} {ci:>18} {row['prob_profitable']:>10.0%}"
            )
        ...
        try:
            ...
            for ch in mmm.channel_names:
                ...
                roi_samples = (
                    contrib_samples / spend
                    if spend > 0
                    else np.zeros_like(contrib_samples)
                )
    ...
    try:
        ...
        for c in comparisons[:5]:  # Show first 5
            ...
            print(
                f"  {c.parameter:<20} {prior_sd:>10} {c.posterior_sd:>10.3f} {shrink:>10}"
            )
    except Exception as e:
        ...
        try:
            ...
            params_to_check = [f"beta_{ch}" for ch in mmm.channel_names] + [
                "intercept",
                "sigma",
            ]
            ...
            if post_summary:
                ...
                for p in post_summary[:5]:
                    print(
                        f"  {p['parameter']:<20} {p['posterior_mean']:>12.4f} {p['posterior_sd']:>10.4f}"
                    )
            else:
    ...
    try:
        ...
        for ch, curve in sat_curves.items():
            print(
                f"  {ch:<15} ${curve.current_spend:>14,.0f} {curve.saturation_level:>11.0%} "
        ...
        try:
            ...
            if sat_info:
                ...
                for ch, info in sat_info.items():
                    print(
                        f"  {ch:<15} {info['lam_mean']:>15.3f} ± {info['lam_std']:.3f}"
                    )
            else:
    ...
    try:
        ...
        for ch, result in adstock_weights.items():
            print(
                f"  {ch:<15} {result.alpha_mean:>10.3f} {result.half_life:>11.1f}w "
        ...
        try:
            ...
            if adstock_info:
                ...
                for ch, info in adstock_info.items():
                    print(
                        f"  {ch:<15} {info['alpha_mean']:>10.3f} {info['half_life']:>11.1f}w"
                    )
            else:
    ...
    try:
        ...
        for d in decomp:
            pct = (
                d.total_contribution / total_contribution
    ...
    try:
        ...
        metrics["predictions"] = {
            ...
            "y_pred_mean": y_pred,
            "y_pred_lower": (
                pred.y_pred_hdi_low if hasattr(pred, "y_pred_hdi_low") else None
            ),
            "y_pred_upper": (
                pred.y_pred_hdi_high if hasattr(pred, "y_pred_hdi_high") else None
            ),
        }
    except Exception as e:
...
def print_key_findings(metrics: dict):
    ...
    if metrics.get("roi") is not None:
        ...
        for _, row in roi_sorted.iterrows():
            ...
            print(
                f"  {row['channel']}: ROI = {row['roi_mean']:.2f}x "
    ...
    if metrics.get("adstock"):
        for ch, result in metrics["adstock"].items():
            ...
            if isinstance(result, dict):
                ...
                print(
                    f"  {ch}: Half-life = {result.half_life:.1f} weeks, "
...
def quick_national_model(n_weeks: int = 52) -> tuple:
    ...
    config = (
        ...
        .add_control_builder(ControlVariableConfigBuilder("Distribution").national())
        .build()
    )
