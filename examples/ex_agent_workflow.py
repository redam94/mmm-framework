...
def main():
    ...
    console.print(
        "[bold green]User:[/bold green] I want to build a Marketing Mix Model, but I don't have data. Can you generate some and run a model?"
    )
        ...
        for event in agent_graph.stream(
            ...
            if hasattr(latest_message, "tool_calls") and latest_message.tool_calls:
                for tc in latest_message.tool_calls:
                    console.print(
                        f"[bold magenta][Tool Call][/bold magenta] {tc['name']}({tc['args']})"
                    )
            ...
            if latest_message.type == "tool":
                console.print(
                    f"[bold blue][Tool Result][/bold blue] {latest_message.name} completed.\n"
                )
                markdown_output.append(
                    f"> **Tool Result:** `{latest_message.name}` completed.\n"
                )

        # Ask user for next input
        console.print("\n[bold cyan]" + "-" * 60 + "[/bold cyan]")
        user_input = console.input(
            "[bold green]You (type 'exit' to stop):[/bold green] "
        )
    ...
    if final_state.get("report_path"):
        console.print(
            f"Report Location: [green]{final_state.get('report_path')}[/green]"
        )
