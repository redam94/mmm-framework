...
def example_multivariate_model():
    ...
    print(
        f"  Cross-effects: {[(c.source_outcome, '→', c.target_outcome, c.effect_type.value) for c in mv_config.cross_effects]}"
    )
...
def example_combined_model():
    ...
    config = (
        ...
        .with_cannibalization(
            source="multipack", target="single_pack", promotion_column="multipack_promo"
        )
        # Configure correlations
        .with_lkj_eta(2.0).build()
    )
    ...
    print(
        f"  Cross-effects: {[(c.source_outcome, '→', c.target_outcome) for c in config.multivariate.cross_effects]}"
    )
...
def example_custom_configurations():
    ...
    print(
        f"  Brand media: l_max={brand_adstock.l_max}, prior=Beta({brand_adstock.prior_alpha}, {brand_adstock.prior_beta})"
    )
    print(
        f"  Perf media: l_max={perf_adstock.l_max}, prior=Beta({perf_adstock.prior_alpha}, {perf_adstock.prior_beta})"
    )

    # Custom effect prior: positive with tight regularization
    tight_positive = (
        EffectPriorConfigBuilder().positive(sigma=0.3).with_tight_prior().build()
    )
