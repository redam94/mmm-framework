...
def render_results_tab():
    ...
    tab_list.extend(["🔮 Scenario Planning", "📋 Summary"])
...
def render_prior_vs_posterior(results, mmm):
    ...
    if st.session_state.prior_samples is None:
        with st.spinner("Sampling from prior distribution..."):
            try:
                st.session_state.prior_samples = mmm.sample_prior_predictive(
                    samples=1000
                )
            except Exception as e:
    ...
    selected_category = st.selectbox(
        "Parameter Category", options=list(param_categories.keys())
    )
    ...
    available_params = [
        p for p in params_to_plot if p in posterior and p in prior.prior
    ]
        ...
        summary_data.append(
            {
                ...
                "Std Reduction (%)": (
                    1 - posterior_samples.std() / (prior_samples.std() + 1e-8)
                )
                * 100,
            }
        )
...
def render_component_breakdown(results, mmm):
    ...
    if st.session_state.component_decomposition is None:
        with st.spinner("Computing component decomposition..."):
            try:
                st.session_state.component_decomposition = (
    ...
    with col2:
        ...
        media_summary["Total Contribution"] = media_summary["Total Contribution"].apply(
            lambda x: f"{x:,.0f}"
        )
        media_summary["Share of Media %"] = media_summary["Share of Media %"].apply(
            lambda x: f"{x:.1f}%"
        )
...
def render_trend_tab(results, mmm, decomp, dim_info, posterior):
    ...
    if trend_type == "Linear":
            ...
            if len(changepoints) > 0:
                ...
                fig_cp.add_trace(
                    go.Bar(
                        ...
                        y=cp_df["Magnitude"],
                        marker_color=[
                            "green" if m > 0 else "red" for m in cp_df["Magnitude"]
                        ],
                    )
                )
        ...
        if "gp_lengthscale" in posterior:
            ...
            amp_samples = (
                posterior["gp_amplitude"].values.flatten()
                if "gp_amplitude" in posterior
                else None
            )
...
def render_seasonality_tab(results, mmm, decomp, dim_info, posterior):
    ...
    if len(periods) >= 52:
        ...
        season_df = pd.DataFrame({"week": weeks, "seasonality": season_values})
        ...
        fig_weekly.add_trace(
            go.Bar(
                ...
                marker_color=[
                    "green" if s > 0 else "red" for s in weekly_pattern["seasonality"]
                ],
            )
        )
