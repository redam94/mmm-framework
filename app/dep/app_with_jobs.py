...
def render_model_tab():
    ...
    with col1:
        if st.button("🚀 Submit Job (Background)", type="primary", use_container_width=True):
...
def submit_background_job(job_name, job_description, n_chains, n_draws, n_tune, 
                          target_accept, trend_type, trend_settings, yearly_order,
                          pool_geo, use_numpyro):
...
def fit_model_inline(n_chains, n_draws, n_tune, target_accept, trend_type, 
                     trend_settings, yearly_order, pool_geo, use_numpyro):
    """Fit the model inline (blocking) - for quick tests."""
    from mmm_framework import (
        BayesianMMM, TrendConfig, TrendType,
        ModelConfigBuilder, HierarchicalConfigBuilder, 
        SeasonalityConfigBuilder, TrendConfigBuilder
    )
...
def render_jobs_tab():
    ...
    failed_jobs = [j for j in all_jobs if j.status in [JobStatus.FAILED, JobStatus.CANCELLED]]
    ...
    with col3:
        st.metric("Completed", len(completed_jobs), delta="✅" if completed_jobs else None)
    with col4:
        st.metric("Failed", len(failed_jobs), delta="❌" if failed_jobs else None, delta_color="inverse")
    
    st.markdown("---")
    
    # Tabs for different job states
    job_tabs = st.tabs(["🔄 Active Jobs", "✅ Completed", "❌ Failed/Cancelled", "📋 All Jobs"])
    
    with job_tabs[0]:
        render_job_list(active_jobs, manager, show_progress=True, key_prefix="active")
    
    with job_tabs[1]:
        render_job_list(completed_jobs, manager, allow_load=True, key_prefix="completed")
...
def render_job_list(jobs, manager, show_progress=False, allow_load=False, 
                   show_error=False, show_all_columns=False, key_prefix=""):
    ...
    for job in jobs:
        with st.container():
            render_job_card(job, manager, show_progress, allow_load, show_error, key_prefix)
            st.markdown("---")


def render_job_card(job, manager, show_progress=False, allow_load=False, show_error=False, key_prefix=""):
    ...
    with col2:
        ...
        if job.result and job.status == JobStatus.COMPLETED:
            st.caption(f"R²: {job.result.r_squared:.3f} | Divergences: {job.result.divergences}")
...
def render_results_tab():
    ...
    if st.session_state.loaded_results is None:
        st.warning("⚠️ No results loaded. Either fit a model inline or load results from a completed job.")
...
def render_summary(results, mmm):
    ...
    key_params = [p for p in summary.index if any(x in p for x in ['beta', 'sigma', 'adstock', 'sat_lam'])]
    ...
    with col2:
        if st.session_state.loaded_contributions is not None:
            csv_contrib = st.session_state.loaded_contributions.channel_contributions.to_csv()
            st.download_button(
