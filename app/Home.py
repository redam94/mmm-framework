...
def main():
    ...
    if not st.session_state.api_connected:
        st.error("⚠️ Cannot connect to the API server. Please ensure the backend is running.")
        st.markdown("""
        ### Getting Started
    ...
    try:
        ...
        with col4:
            ...
            st.metric("Active Jobs", active, delta=f"{pending} pending" if pending > 0 else None)
    ...
    try:
        ...
        if models:
            ...
            st.info("No models found. Get started by uploading data and creating a configuration.")
