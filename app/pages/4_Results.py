...
def render_diagnostics_tab(model_id: str):
    ...
    try:
        ...
        if divergences > 0:
            issues.append(
                f"⚠️ {divergences} divergences detected. Consider increasing `target_accept` or reparameterizing the model."
            )
        if rhat > 1.01:
            issues.append(
                f"⚠️ R-hat ({rhat:.3f}) exceeds 1.01. Chains may not have converged. Consider running more iterations."
            )
        if ess < 400:
            issues.append(
                f"⚠️ Low ESS ({ess:.0f}). Effective sample size is low. Consider running more iterations."
            )
        ...
        if issues:
            ...
            st.success(
                "✅ All diagnostics look good! The model appears to have converged properly."
            )
...
def render_geo_comparison(fit_data: dict, periods: list, geographies: list):
    ...
    if not df_metrics.empty:
        ...
        if "MAPE (%)" in df_display.columns:
            df_display["MAPE (%)"] = df_display["MAPE (%)"].apply(
                lambda x: f"{x:.2f}" if x is not None else "N/A"
            )
    ...
    try:
        ...
        with col1:
            if r2 is not None:
                ...
                if r2 >= 0.9:
                    st.metric(
                        "R²", f"{r2:.4f}", delta="Excellent", delta_color="normal"
                    )
                elif r2 >= 0.7:
        ...
        with col3:
            if mape is not None:
                if mape < 10:
                    st.metric(
                        "MAPE", f"{mape:.2f}%", delta="Excellent", delta_color="normal"
                    )
                elif mape < 20:
                    st.metric(
                        "MAPE", f"{mape:.2f}%", delta="Good", delta_color="normal"
                    )
                else:
                    st.metric(
                        "MAPE", f"{mape:.2f}%", delta="High", delta_color="inverse"
                    )
        with col4:
            ...
            if predicted_std:
                ...
                within_band = sum(
                    1
                    for o, m, s in zip(observed, predicted_mean, predicted_std)
        ...
        fig.update_layout(
            ...
            yaxis_title=(
                "Sales"
                if selected_view == "Aggregated (Total)"
                else f"Sales ({geo_name})" if has_geo else "Sales"
            ),
            height=450,
            legend=dict(
                orientation="h",
        ...
        if predicted_mean:
            ...
            with st.expander("📊 Residual Analysis", expanded=False):
                ...
                st.plotly_chart(
                    fig_resid, use_container_width=True, key=f"resid_{selected_view}"
                )
...
def render_posteriors_tab(model_id: str):
    ...
    try:
        ...
        adstock_params = [
            p for p in all_params if "adstock" in p.lower() or "alpha" in p.lower()
        ]
        saturation_params = [
            p for p in all_params if "sat" in p.lower() or "lam" in p.lower()
        ]
        other_params = [
            p
            for p in all_params
            if p not in beta_params + adstock_params + saturation_params
        ]
        ...
        with tabs[1]:
            if beta_params:
                plot_posterior_distributions(
                    {k: posteriors_data[k] for k in beta_params}
                )
            else:
        ...
        with tabs[2]:
            if adstock_params:
                plot_posterior_distributions(
                    {k: posteriors_data[k] for k in adstock_params}
                )
            else:
        ...
        with tabs[3]:
            if saturation_params:
                plot_posterior_distributions(
                    {k: posteriors_data[k] for k in saturation_params}
                )
            else:
        ...
        with tabs[4]:
            if other_params:
                plot_posterior_distributions(
                    {k: posteriors_data[k] for k in other_params}
                )
            else:
...
def render_prior_posterior_tab(model_id: str):
    ...
    try:
        ...
        if not params_with_prior:
            st.warning(
                f"No parameters in '{selected_category}' have prior samples available."
            )
        ...
        st.plotly_chart(
            fig, use_container_width=True, key=f"pp_plot_{selected_category}"
        )
        ...
        if shrinkage_data:
            ...
            # Add reference lines
            fig_shrinkage.add_vline(
                x=40, line_dash="dash", line_color="gray", opacity=0.5
            )
            fig_shrinkage.add_vline(
                x=70, line_dash="dash", line_color="gray", opacity=0.5
            )

            st.plotly_chart(
                fig_shrinkage,
                use_container_width=True,
                key=f"shrinkage_{selected_category}",
            )
...
def render_response_curves_tab(model_id: str):
    ...
    try:
        ...
        for channel, data in channels.items():
            ...
            current_idx = min(
                range(len(spend)), key=lambda i: abs(spend[i] - current_spend)
            )
            response_at_current = (
                response[current_idx] if current_idx < len(response) else 0
            )
            ...
            saturation_at_current = (
                (response_at_current / max_response * 100) if max_response > 0 else 0
            )
            saturation_at_max = (
                (response_at_max / max_response * 100) if max_response > 0 else 0
            )
        ...
        if roas_data and roas_data.get("channels"):
            ...
            st.dataframe(
                pd.DataFrame(roas_table), use_container_width=True, hide_index=True
            )
...
def render_contributions_tab(model_id: str):
    ...
    with col2:
        compute_btn = st.button(
            "🔄 Compute", type="primary", key="compute_contributions_btn"
        )
    ...
    try:
        ...
        if compute_btn:
            with st.spinner("Computing contributions..."):
                contributions = client.compute_contributions(
                    model_id, hdi_prob=hdi_prob
                )
        else:
        ...
        with viz_tabs[3]:
                ...
                [
                    {
                        "Channel": k,
                        "Contribution": v,
                        "Percentage": contrib_pct.get(
                            k,
                            (
                                v / sum(total_contrib.values())
                                if sum(total_contrib.values()) > 0
                                else 0
                            ),
                        ),
                    }
            ...
            contrib_df["Contribution"] = contrib_df["Contribution"].apply(
                lambda x: f"{x:,.0f}"
            )
            contrib_df["Percentage"] = contrib_df["Percentage"].apply(
                lambda x: f"{x*100:.1f}%"
            )
...
def render_trend_seasonality_tab(model_id: str):
    ...
    try:
        ...
        has_trend = metadata.get("has_trend", False) or np.any(
            np.array(trend_data) != 0
        )
        has_seasonality = metadata.get("has_seasonality", False) or np.any(
            np.array(seasonality_data) != 0
        )
        ...
        with col2:
            if has_trend:
                trend_change = (
                    trend_data[-1] - trend_data[0] if len(trend_data) > 1 else 0
                )
                st.metric(
                    "Total Trend Change",
                    f"{trend_change:,.0f}",
        ...
        with ts_tabs[0]:
            ...
            if not has_trend:
                st.info(
                    "No trend component was included in this model (trend_type = 'none')."
                )
            else:
                # Trend over time
                fig_trend = go.Figure()
        ...
        with ts_tabs[1]:
            ...
            if not has_seasonality:
                st.info(
                    "No seasonality component was included in this model (yearly_seasonality = 0)."
                )
            else:
                ...
                st.plotly_chart(
                    fig_seas, use_container_width=True, key="seasonality_main"
                )
                ...
                if n_periods >= 52:
                    ...
                    if n_complete_years >= 1:
                        ...
                        with col1:
                            st.success(
                                f"📈 **Peak Season**: Week {peak_week} (+{seasonal_pattern[peak_week-1]:,.0f})"
                            )
                        with col2:
                            st.error(
                                f"📉 **Low Season**: Week {trough_week} ({seasonal_pattern[trough_week-1]:,.0f})"
                            )
        ...
        with ts_tabs[2]:
            ...
            if len(geo_names) <= 1:
                st.info(
                    "This model has only one geography. Geographic breakdown is not applicable."
                )
            else:
                ...
                if not available_geo_components:
                    ...
                    if component_select in by_geography:
                        ...
                        st.plotly_chart(
                            fig_geo, use_container_width=True, key="geo_trend_seas"
                        )
                        ...
                        st.warning(
                            f"{component_select.title()} not available by geography."
                        )
        ...
        with ts_tabs[3]:
            ...
            for i in range(1, 4):
                fig_combined.add_hline(
                    y=0, line_dash="dash", line_color="lightgray", row=i, col=1
                )
            ...
            summary_data = [
                {
                    "Component": "Baseline",
                    "Total": f"{total_baseline:,.0f}",
                    "% of Observed": (
                        f"{total_baseline/total_observed*100:.1f}%"
                        if total_observed
                        else "N/A"
                    ),
                },
                {
                    "Component": "Trend",
                    "Total": f"{total_trend:,.0f}",
                    "% of Observed": (
                        f"{total_trend/total_observed*100:.1f}%"
                        if total_observed
                        else "N/A"
                    ),
                },
                {
                    ...
                    "Total": f"{total_seasonality:,.0f}",
                    "% of Observed": (
                        f"{total_seasonality/total_observed*100:.1f}%"
                        if total_observed
                        else "N/A"
                    ),
                },
    ...
    try:
        ...
        with filter_cols[0]:
            view_level = st.radio(
                "View Level",
                options=(
                    ["Aggregate", "By Geography", "By Product"]
        ...
        with filter_cols[1]:
            if view_level == "By Geography" and has_geo:
                selected_geo = st.selectbox(
                    "Select Geography", options=geo_names, key="decomp_geo_filter"
                )
            elif has_geo:
        ...
        with filter_cols[2]:
            if view_level == "By Product" and has_product:
                selected_product = st.selectbox(
                    "Select Product", options=product_names, key="decomp_product_filter"
                )
            elif has_product:
        ...
        def get_observed_data():
            ...
            if (
                view_level == "By Geography"
                and selected_geo
                and selected_geo in observed_by_geo
            ):
                return np.array(observed_by_geo[selected_geo])
            elif (
                view_level == "By Product"
                and selected_product
                and selected_product in observed_by_prod
            ):
        ...
        with st.expander("🔍 Debug: Available Components", expanded=False):
            ...
            for k, v in components.items():
                if isinstance(v, list):
                    st.write(
                        f"  - {k}: {len(v)} values, range [{min(v):.2f}, {max(v):.2f}]"
                    )
        ...
        predicted = (
            np.array(baseline_comp)
            + np.array(trend_comp)
            + np.array(seasonality_comp)
            + media_total
            + controls_total
            + np.array(geo_effects)
            + np.array(product_effects)
        )
        ...
        if not has_media_data and not media_components:
            st.warning(
                "⚠️ No media channel contributions found in the decomposition. This may indicate the model's media contribution variables aren't being captured correctly."
            )
        ...
        with decomp_tabs[0]:
            ...
            with col2:
                ...
                display_df["Total Contribution"] = display_df[
                    "Total Contribution"
                ].apply(lambda x: f"{x:,.0f}")
                display_df["% of Predicted"] = display_df["% of Predicted"].apply(
                    lambda x: f"{x:.1f}%"
                )
                ...
                if media_components:
                    ...
                    for name, values in media_components.items():
                        ...
                        total = np.sum(values)
                        pct_of_media = (
                            (total / np.sum(media_total) * 100)
                            if np.sum(media_total) != 0
                            else 0
                        )
        ...
        with decomp_tabs[1]:
            ...
            if components_added == 0:
                ...
                st.plotly_chart(
                    fig_stack, use_container_width=True, key="decomp_stacked"
                )

                # Show individual media channels option
                if media_components:
                    with st.expander(
                        "📺 Show Individual Media Channels", expanded=False
                    ):
        ...
        with decomp_tabs[2]:
            ...
            all_components.extend(
                [f"Media: {k.replace('media_', '')}" for k in media_components.keys()]
            )
            all_components.extend(
                [
                    f"Control: {k.replace('control_', '')}"
                    for k in control_components.keys()
                ]
            )
            ...
            if selected:
                ...
                for k, v in control_components.items():
                    component_map[f"Control: {k.replace('control_', '')}"] = (
                        v,
                        "#f39c12",
                    )
        ...
        with decomp_tabs[3]:
            ...
            if len(geo_names) <= 1 or not by_geography:
                st.info(
                    "Geographic breakdown requires multiple geographies and geo-level data from the model."
                )
            else:
                ...
                # Add media channels
                media_geo_keys = [
                    k for k in by_geography.keys() if k.startswith("media_")
                ]
                ...
                # Add controls
                control_geo_keys = [
                    k for k in by_geography.keys() if k.startswith("control_")
                ]
                for k in control_geo_keys:
                    geo_component_options.append(
                        f"Control: {k.replace('control_', '')}"
                    )
                ...
                if not geo_component_options:
                    ...
                    if selected_geo_comp == "Baseline":
                        ...
                        geo_key = (
                            f"control_{selected_geo_comp.replace('Control: ', '')}"
                        )
                    else:
                        geo_key = None
                    ...
                    if geo_key and geo_key in by_geography:
                        ...
                        for i, (geo, values) in enumerate(geo_data.items()):
                            fig_geo_decomp.add_trace(
                                go.Scatter(
                                    ...
                                    name=geo,
                                    line=dict(
                                        color=geo_colors[i % len(geo_colors)], width=2
                                    ),
                        ...
                        # Stacked area by geo
                        with st.expander(
                            "📊 Stacked Area by Geography", expanded=False
                        ):
                            fig_geo_stack = go.Figure()

                            for i, (geo, values) in enumerate(geo_data.items()):
                                fig_geo_stack.add_trace(
                                    go.Scatter(
                                        x=periods_display,
                                        y=values,
                                        name=geo,
                            ...
                            st.plotly_chart(
                                fig_geo_stack,
                                use_container_width=True,
                                key="decomp_geo_stack",
                            )

                        # Observed by geography if available
                        if observed_by_geo:
                            with st.expander(
                                "📈 Observed Values by Geography", expanded=False
                            ):
                                fig_obs_geo = go.Figure()

                                for i, (geo, values) in enumerate(
                                    observed_by_geo.items()
                                ):
                                    fig_obs_geo.add_trace(
                                        go.Scatter(
                                            x=periods_display,
                                            y=values,
                                            mode="lines",
                ...
                if len(yearly) < 2:
                    ...
                    with col1:
                        base_year = st.selectbox(
                            "Base Year", options=years[:-1], index=0, key="yoy_base"
                        )
                    with col2:
                    ...
                    if compare_year:
                        ...
                        # Calculate changes
                        changes = {
                            "Baseline": compare_data["baseline"]
                            - base_data["baseline"],
                            "Trend": compare_data["trend"] - base_data["trend"],
                            "Seasonality": compare_data["seasonality"]
                            - base_data["seasonality"],
                            "Media": compare_data["media"] - base_data["media"],
                            "Controls": compare_data["controls"]
                            - base_data["controls"],
                        }
                        ...
                        if media_components:
                            ...
                            media_yearly = (
                                df_media.groupby("year")[
                                    [
                                        f'media_{k.replace("media_", "")}'
                                        for k in media_components.keys()
                                    ]
                                ]
                                .sum()
                                .reset_index()
                        ...
                        fig_waterfall = go.Figure(
                            go.Waterfall(
                                ...
                                textposition="outside",
                                text=[
                                    f"{v:+,.0f}" if m == "relative" else f"{v:,.0f}"
                        ...
                        st.plotly_chart(
                            fig_waterfall, use_container_width=True, key="yoy_waterfall"
                        )
                        ...
                        for name, change in changes.items():
                            pct_change = (
                                (change / base_data["observed"] * 100)
                                if base_data["observed"] != 0
                                else 0
                            )
                            change_summary.append(
                                {
                                    "Component": name,
                                    f"{base_year}": (
                                        f"{base_data[name.lower().replace(' ', '_').replace('(total)', '').strip()]:,.0f}"
                                        if name.lower()
                                        .replace(" ", "_")
                                        .replace("(total)", "")
                                        .strip()
                                    ...
                                    f"{compare_year}": (
                                        f"{compare_data[name.lower().replace(' ', '_').replace('(total)', '').strip()]:,.0f}"
                                        if name.lower()
                                        .replace(" ", "_")
                                        .replace("(total)", "")
                                        .strip()
                        ...
                        change_df = pd.DataFrame(
                            [
                                {
                                    "Component": "Baseline",
                                    f"{base_year}": f"{base_data['baseline']:,.0f}",
                                    f"{compare_year}": f"{compare_data['baseline']:,.0f}",
                                    "Change": f"{changes['Baseline']:+,.0f}",
                                    "% of Total Change": (
                                        f"{changes['Baseline']/(compare_total-base_total)*100:.1f}%"
                                        if (compare_total - base_total) != 0
                                        else "0%"
                                    ),
                                },
                                {
                                    "Component": "Trend",
                                    f"{base_year}": f"{base_data['trend']:,.0f}",
                                    f"{compare_year}": f"{compare_data['trend']:,.0f}",
                                    "Change": f"{changes['Trend']:+,.0f}",
                                    "% of Total Change": (
                                        f"{changes['Trend']/(compare_total-base_total)*100:.1f}%"
                                        if (compare_total - base_total) != 0
                                        else "0%"
                                    ),
                                },
                                {
                                    "Component": "Seasonality",
                                    f"{base_year}": f"{base_data['seasonality']:,.0f}",
                                    f"{compare_year}": f"{compare_data['seasonality']:,.0f}",
                                    "Change": f"{changes['Seasonality']:+,.0f}",
                                    "% of Total Change": (
                                        f"{changes['Seasonality']/(compare_total-base_total)*100:.1f}%"
                                        if (compare_total - base_total) != 0
                                        else "0%"
                                    ),
                                },
                                {
                                    "Component": "Media",
                                    f"{base_year}": f"{base_data['media']:,.0f}",
                                    f"{compare_year}": f"{compare_data['media']:,.0f}",
                                    "Change": f"{changes['Media']:+,.0f}",
                                    "% of Total Change": (
                                        f"{changes['Media']/(compare_total-base_total)*100:.1f}%"
                                        if (compare_total - base_total) != 0
                                        else "0%"
                                    ),
                                },
                                {
                                    "Component": "Controls",
                                    f"{base_year}": f"{base_data['controls']:,.0f}",
                                    f"{compare_year}": f"{compare_data['controls']:,.0f}",
                                    "Change": f"{changes['Controls']:+,.0f}",
                                    "% of Total Change": (
                                        f"{changes['Controls']/(compare_total-base_total)*100:.1f}%"
                                        if (compare_total - base_total) != 0
                                        else "0%"
                                    ),
                                },
                        ...
                        st.dataframe(
                            change_df, use_container_width=True, hide_index=True
                        )

                        # Total change
                        total_change = compare_total - base_total
                        pct_total = (
                            (total_change / base_total * 100) if base_total != 0 else 0
                        )
        ...
        with decomp_tabs[5]:
            ...
            stack_order_rel = [
                (
                    "Baseline",
                    np.array(baseline_comp) / total_by_period * 100,
                    "#3498db",
                ),
                ("Trend", np.array(trend_comp) / total_by_period * 100, "#e74c3c"),
                (
                    "Seasonality",
                    np.array(seasonality_comp) / total_by_period * 100,
                    "#2ecc71",
                ),
...
def render_summary_tab(model_id: str):
    ...
    try:
        ...
        with col3:
            if st.button(
                "📥 Download Model", use_container_width=True, key="download_model_btn"
            ):
...
def main():
    """Main page function."""
    page_header(
        "📈 Results", "View model diagnostics, contributions, and analysis results."
    )
