...
def render_scenario_form():
    ...
    if scenario_type == "Percentage Change":
        ...
        current_pcts = {ch: (v / total_budget * 100) if total_budget > 0 else (100 / len(channels)) 
    ...
    if use_time_period:
        ...
        with col2:
            end_idx = st.number_input("End Index", min_value=1, value=results.get("n_obs", 100))
...
def render_scenario_results():
    ...
    with col3:
        ...
        st.metric("Change", format_number(change), delta=f"{change_pct:+.1f}%", delta_color=delta_color)
    with col4:
        roi = change / sum(abs(v * baseline) for v in results.get("spend_changes", {}).values()) if results.get("spend_changes") else 0
...
def render_scenario_history():
    ...
    for i, entry in enumerate(reversed(history)):
        ...
        with st.expander(f"Scenario {len(history) - i}: {entry['type']}", expanded=i==0):
...
def main():
    ...
    page_header(
        "🔮 Scenarios",
        "Run what-if scenarios and optimize your marketing budget."
    )
