...
def render_job_form():
    ...
    with st.form("fit_form"):
        ...
        with col2:
            ...
            with col_a:
                override_chains = st.number_input("Chains", min_value=1, max_value=8, value=None, placeholder="Default")
            with col_b:
                override_draws = st.number_input("Draws", min_value=100, max_value=10000, value=None, placeholder="Default")
            with col_c:
                override_tune = st.number_input("Tune", min_value=100, max_value=5000, value=None, placeholder="Default")
            
            random_seed = st.number_input("Random Seed (optional)", min_value=0, value=None, placeholder="Random")
        
        # Submit button
        submitted = st.form_submit_button("🚀 Start Fitting", type="primary", use_container_width=True)
    ...
    try:
        ...
        with col3:
            if model.completed_at:
                ...
                duration = (model.completed_at - model.started_at).total_seconds() if model.started_at else None
                if duration:
    ...
    try:
        ...
        for model in models:
            with st.container():
                ...
                with col1:
                    is_monitoring = st.session_state.monitoring_model_id == model.model_id
...
def main():
    ...
    page_header(
        "🔬 Model Fitting",
        "Submit and monitor Bayesian MMM fitting jobs."
    )
    ...
    with col_left:
        with st.expander("➕ Submit New Job", expanded=not st.session_state.monitoring_model_id):
            render_job_form()
