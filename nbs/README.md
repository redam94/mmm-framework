...
| `showcase/` | **The feature showcase** (00–06): the whole framework, chart-first, one sitting per notebook — 00 the measurement loop end-to-end in miniature (fit → ROI vs planted truth → report → serialize, plus the package map); 01 data foundations (MFF, bundled data, synth worlds as a feature, EDA, Data Studio, Excel config, native datasets); 02 model anatomy (every adstock/saturation/trend/seasonality family charted, ROI-mode priors, levers, all NUTS backends + approximate fits, the frequentist path); 03 beyond one KPI (Nested/Multivariate/Combined extensions, DAG-driven building, Model Garden, CFA, LTV, causal estimators); 04 earning trust (the three ROI estimands side by side, prior→posterior learning, confounding tipping points on a world where the model genuinely over-credits, backtest, spec curve, refutation, experiment calibration); 05 plan & commit (valuation refusals, optimizer + frontier, promo arms, forecast, plan-of-record reproduced to 1e-9, payback, pacing, the variance waterfall); 06 operate the loop (EIG/EVOI, experiment economics + Pareto optimizer, the model-free geo bandit, prefit/interactive reporting, jobs, security/auth/platform services, the agent architecture) | — |
| `demos/` | Standalone walkthroughs: modeling workflow, messy-data onboarding, causal features showcase, PPTX deck demo, every fit method compared (MAP/Laplace/ADVI/Pathfinder/SMC vs NUTS), NUTS backends compared (pymc/numpyro/nutpie — agreement, ESS/sec, scale-dependent ranking), **paradigms** compared (ridge/bootstrap vs MAP vs NUTS on planted truth — what ridge buys over MAP, what a bootstrap confidence interval means, when a hard constraint justifies CVXPY, and where the frequentist path fails *worse*), experiment-planning playbook (method registry → power → EIG/EVOI → net-value Pareto optimizer → calibration), LTV problems & the Atelier models (BG/NBD+Gamma-Gamma CLV, binomial awareness, CFA, LCA, long-term brand — each on synthetic data with a known answer key), **critique → decision** (model critique, specification search, optimization toward different estimands, and experimental measurement on one confounded world with a sealed answer key — the fit grades *excellent* and its best channel is the truth's third-best), **payback horizon** (t50/t90 response timing per posterior draw against a sealed carryover answer key: the truncation illusion measured, the prior-domination gate, the misspec negative control firing SHORT, refusals by name, and the finance break-even that refuses without a valuation), **promo-depth optimization** (decision arms with per-arm cost bases on the `promo_and_media` world: promo ROI with its refusals, the cost-space reduction, the joint solve graded on true planted profit, the price what-if that refuses to recommend, and the clearance-timing negative control), **variance to plan** (commit a reproducible forecast snapshot, let the season play out in a world with a causal answer key, then build the two-bucket bridge — delivery variance on the committed posterior graded against `response_fn` truth, a labelled unexplained remainder, supplied lines with required source notes, the refit "effectiveness" split refused, and the committed-interval verdict leading) | — |
