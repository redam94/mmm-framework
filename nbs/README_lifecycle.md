...
[`continuous_learning_story`](continuous_learning_story.ipynb) notebook: same loop,
...
Everything lives in **`nbs/lifecycle_common.py`** (a plain module, imported as `L`,
...
- [`continuous_learning_story`](continuous_learning_story.ipynb) — the same loop run
  **model-free** (a geo response-surface bandit, no MMM).
- [`workshop_05_from_draws_to_decisions`](workshop_05_from_draws_to_decisions.ipynb) —
  the marginal-vs-average-ROAS and reallocation-under-uncertainty pedagogy in depth.
- [`math_05_calibration`](math_05_calibration.ipynb) — the calibration mathematics
