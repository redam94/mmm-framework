...
Lifecycle series (run from ``nbs/``).
    ...
    uv run --with nbformat python build_lifecycle_05_allocate.py
    TQDM_DISABLE=1 PYTHONPATH=.. uv run --with nbconvert --with nbformat \
        ...
        lifecycle_05_allocate.ipynb --ExecutePreprocessor.timeout=2400 \
...
palette live in ``nbs/lifecycle_common.py``.
"""
...
def main() -> None:
    ...
    out = "lifecycle_05_allocate.ipynb"
