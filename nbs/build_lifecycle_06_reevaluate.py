...
Measurement Lifecycle series (run from ``nbs/``).
    ...
    uv run --with nbformat python build_lifecycle_06_reevaluate.py
    TQDM_DISABLE=1 PYTHONPATH=.. uv run --with nbconvert --with nbformat \
        ...
        lifecycle_06_reevaluate.ipynb --ExecutePreprocessor.timeout=2400 \
...
Shared world / fit / palette live in ``nbs/lifecycle_common.py``.
"""
...
def main() -> None:
    ...
    out = "lifecycle_06_reevaluate.ipynb"
