"""Author lifecycle_04_calibrate.ipynb — T3 of the series (run from ``nbs/``).
    ...
    uv run --with nbformat python build_lifecycle_04_calibrate.py
    TQDM_DISABLE=1 PYTHONPATH=.. uv run --with nbconvert --with nbformat --with ipykernel \
        jupyter nbconvert --to notebook --execute --inplace \
        lifecycle_04_calibrate.ipynb --ExecutePreprocessor.timeout=2400 \
...
stay put. Shared world/fit/palette live in ``nbs/lifecycle_common.py``.
"""
...
def main() -> None:
    ...
    out = "lifecycle_04_calibrate.ipynb"
