"""Author stress notebook 04 — extension traps (run from ``nbs/``).
    ...
    uv run python builders/build_stress_04_extension_traps.py
    PYTHONPATH=.. uv run jupyter nbconvert --to notebook --execute --inplace \
        stress/stress_04_extension_traps.ipynb --ExecutePreprocessor.timeout=3600
...
Everything is graded against the **Aurora** world (`nbs/builders/aurora.py`), whose
generative truth — per-channel ROAS, mediated share, the cannibalization
mechanism — is known exactly.

> Companions: `../math/math_06_extensions.ipynb` (the extension equations, derived and
...
1. **The estimand.** The outcome equation routes TV and Display through a latent
   ...
   `../math/math_06_extensions.ipynb` §2.)
...
def main():
    ...
    path = "stress/stress_04_extension_traps.ipynb"
