"""Author stress_05_the_gauntlet.ipynb — the capstone of the stress series (run from ``nbs/``).
    ...
    uv run python builders/build_stress_05_gauntlet.py
    PYTHONPATH=.. uv run jupyter nbconvert --to notebook --execute --inplace \
        stress/stress_05_the_gauntlet.ipynb --ExecutePreprocessor.timeout=3600
...
`nbs/builders/aurora.py`): Aurora Coffee Co., two years of weekly data, four channels —
...
> [mmm_walkthrough](../demos/mmm_walkthrough.ipynb) the same workflow on a tamer world.
"""),
    code(r"""
...
The pre-flight checklist from [mmm_walkthrough Part 1](../demos/mmm_walkthrough.ipynb)
...
[math_05](../math/math_05_calibration.ipynb)) and refit. In this synthetic world the
...
- The *direction* of the budget implication — the channels whose estimates
  ...
  [05_unified_workflow](../aurora/05_unified_workflow.ipynb)) values correcting that
...
[mmm_walkthrough](../demos/mmm_walkthrough.ipynb) (the same workflow on a tamer
realistic world) · `tests/synth/` (the stress matrix this series is built on) ·
[math_05](../math/math_05_calibration.ipynb) (the calibration algebra).
"""),
]
...
def main():
    ...
    path = "stress/stress_05_the_gauntlet.ipynb"
