"""Author the continuous-learning notebook (run from ``nbs/``).
    ...
    uv run --with nbformat python builders/build_continuous_learning.py
    PYTHONPATH=.. uv run --with nbconvert --with nbformat --with ipykernel \
        jupyter nbconvert --to notebook --execute --inplace \
        continuous_learning/continuous_learning.ipynb --ExecutePreprocessor.timeout=3600 \
...
"""),
    code(r"""
import sys
sys.path.insert(0, "../builders")           # make the sibling builder importable in-kernel
import jax as _jax
...
def main() -> None:
    ...
    with open("continuous_learning/continuous_learning.ipynb", "w") as fh:
