"""Author the breakout-weighting notebook (run from ``nbs/``).
    ...
    uv run python builders/build_breakout_weighting.py
    PYTHONPATH=.. uv run jupyter nbconvert --to notebook --execute --inplace \
        extensions/breakout_weighting.ipynb --ExecutePreprocessor.timeout=3600
...
ROOT = next(p for p in (pathlib.Path.cwd(), *pathlib.Path.cwd().parents)
            if (p / "src" / "mmm_framework").exists())  # repo root, wherever the kernel runs
...
def main():
    ...
    path = "extensions/breakout_weighting.ipynb"
